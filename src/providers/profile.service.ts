...
export class ProfileService {
  ...
  * @param ref - Profile database reference path
  */
  saveProfile(data, ref?){
    return this._auth.getCurrentUser().then((currentUser) => {
      let profile_ref = this.PROFILE_REF + currentUser.uid;
      if(ref) {  profile_ref + "/" + ref  }
      data.uid = currentUser.uid;
      return this._dataService.database.child(profile_ref).update(data);
    });
  }
